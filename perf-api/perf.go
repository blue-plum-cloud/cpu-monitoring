...
func ConfigPerf(pe * C.struct_perf_event_attr, pec * PerfEventConfig, cpu int) (C.int){
	group_fd := C.config_perf_multi(
		(*C.struct_perf_event_attr)(unsafe.Pointer(pe)), // Assuming `pe` is correctly initialized
		&pec.Fds[0],
		&pec.Ids[0],
		&pec.Types[0],
		&pec.Configs[0],
...
// 	var pe C.struct_perf_event_attr
...
// 	}
// 	group_fd := ConfigPerf(&pe, &peConfig)
...
// 	EndInstrumentation(&pe, group_fd, cStrings, &peConfig, numEvents)
